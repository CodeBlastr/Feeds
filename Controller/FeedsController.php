...
class AppFeedsController extends FeedsAppController {
			...
			try {
				...
				foreach ($this->uses as $k => $model) {
					...
					if (count($model) > 1 && $model[0] == 'Feeds') {
						$products = array_merge($products, $this->$model[1]->find('all', array('conditions' => $conditions, )));
	...
	public function _getProductRating($id, $fromUsers = false) {
		...
		$ratings['Ratings'] = $this->Rating->find('all', array('conditions' => $conditions, ));
